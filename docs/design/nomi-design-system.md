...
| Token | px | 用途 |
|---|---|---|
| `micro` | 11 | 角标、徽标、零填充编号 |
| `caption` | 12 | hint、占位副标题、列表 metadata |
| `bodySm` | 13 | 表单输入、紧凑列表 |
| `body` | 14 | 正文 |
| `title` | 16 | 卡片标题、面板 header |
| `h2` | 20 | 区域 heading |
| `h1` | 24 | 页面 heading |

行高对应 `nomiDesignTokens.lineHeight`，永远成对使用。

**不允许：** `text-[13.7px]`、`leading-[15px]`、`text-2xl`（除非映射到 token）。
...
文件：`src/workbench/generationCanvasV2/nodes/TitlePill.tsx`
...
文件：`src/workbench/generationCanvasV2/nodes/BaseGenerationNode.tsx`（嵌入式）+ CSS class `.generation-canvas-v2-node__derived-badge`
...
| 背景 | `rgba(255, 255, 255, 0.86)` + backdrop-blur |
| 文字 | `var(--nomi-accent)` |
| 字号 | 10.5px |
| 字重 | 650 |
| 图标 | `IconCopy` 13px stroke 1.8 |
| 文案 | 正文 "独立副本" \| tooltip "独立副本（来自 [分类]·[名]）" |
| 行为 | 点击跳转源节点（支持跨分类切换） |

### 4.4 `GroupFrame`（组框）

文件：`src/workbench/generationCanvasV2/components/GroupFrame.tsx`
...
3b. **然后写代码**：
- 文件放对位置（通用 → `src/design/`、画布 → `src/workbench/generationCanvasV2/`、sidebar → `src/workbench/sidebar/`）
- export 加到对应 index
