...
function defineGenerationNodePlugins<const TPlugins extends readonly [GenerationNodePluginDefinition, ...GenerationNodePluginDefinition[]]>(
  plugins: TPlugins,
): TPlugins {
  return plugins
}

const loadBaseGenerationNode = () => import('./BaseGenerationNode') as Promise<{ default: GenerationNodeComponent }>
...
export const GENERATION_NODE_PLUGIN_BY_KIND: Record<GenerationNodeKind, GenerationNodePlugin> =
  Object.fromEntries(GENERATION_NODE_PLUGINS.map((plugin) => [plugin.kind, plugin])) as Record<GenerationNodeKind, GenerationNodePlugin>
